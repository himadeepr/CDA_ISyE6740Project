...
getTimeDiff <- function() {
    ...
    tmean <- aggregate(time ~ bidder_id,res,  
                       function(x)  mean(diff(sort(as.numeric(x)))))
                           
    tsd <- aggregate(time ~ bidder_id, res, 
                     function(x) sd(diff(sort(as.numeric(x)))))

    tmedian <- aggregate(time ~ bidder_id, res, 
                         function(x) median(diff(sort(as.numeric(x)))))

    tmin <- aggregate(time ~ bidder_id, res, 
                      function(x) min(diff(sort(as.numeric(x)))))
    
    tfast <- aggregate(time ~ bidder_id, res, function(x) {
            dv <- diff(sort(as.numeric(x)))
            md <- mean(dv)
            if (!is.na(md)) {
...
country_crime <- getCountryCrime(5)
